...
class YOLOv8PAFPN_SPLIT(YOLOv5PAFPN):
    ...
    def split_channels(self, inputs: Tensor) -> list:
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        results.append(self.split_channels(outs[-1])[:])
...
class YOLOv8PAFPN_FPN(YOLOv5PAFPN):
    ...
    def __init__(self,
        ...
        super().__init__(
            ...
            init_cfg=init_cfg)
        self.real_out_layers = nn.ModuleList()
        self.real_out_layers.append(mmdet.models.FPN(in_channels=self.in_channels,out_channels=self.real_out_channels[0],num_outs=5))
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        # out_to_fpn
        outputs = self.real_out_layers[0](tuple(results))
...
class YOLOv8PAFPN_AFF(YOLOv5PAFPN):
    ...
    def build_reduce_layer(self, idx: int) -> nn.Module:
        ...
        if idx == len(self.in_channels) - 1:
            ...
            return AFF(channels=self.in_channels[idx])
...
class YOLOv8PAFPN_MSCAM(YOLOv5PAFPN):
    ...
    def __init__(self,
        ...
        self.real_out_layers = nn.ModuleList()
        self.fpn_layers = nn.ModuleList()
        self.ms_cam_layers = nn.ModuleList()
        ...
        for idx in range(len(self.real_out_layers)):
            if idx >= len(self.in_channels):
                self.fpn_layers.append(self.build_fpn_layer(idx))
        for idx in range(len(self.in_channels)-1):
            self.ms_cam_before_layers.append(MS_CAM(channels=self.in_channels[idx]+self.in_channels[idx+1]))
            self.ms_cam_after_layers.append(MS_CAM(channels=self.in_channels[idx]+self.in_channels[idx+1]))
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            top_down_layer_inputs = self.ms_cam_layers[idx](top_down_layer_inputs)
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            out = self.ms_cam_after_layers[idx](out)
...
class YOLOv8PAFPN_SIMPLE_AFF(YOLOv5PAFPN):
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        results = []
        for idx in range(len(self.in_channels)):
            results.append(self.aff_layer(self.out_layers[idx](outs[idx]),inputs[idx]))
